...
import (
	"encoding/json"
	"fmt"
	"github.com/aftaab60/bookstore_oauth-go/oauth/errors"
...
func AuthenticateRequest(request *http.Request) *errors.RestErr {
	...
	request.Header.Set(headerXCallerId, fmt.Sprintf("%v", at.CallerId))
...
func getAccessToken(accessTokenId string) (*accessToken, *errors.RestErr) {
	...
	if response.StatusCode > 299 {
		return nil, errors.NewInternalServerError("Error response when getting access token")
	}
